...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author Mathew
	 **/
	public function login($identity, $password, $remember=FALSE, $is_server=FALSE)
	{
		...
		if((!$is_server)&&$this->is_time_locked_out($identity))
		{
			...
			$this->set_message('timeout_login');
			return FALSE;
		}
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE)
			{
				...
				$this->trigger_events('post_login_unsuccessful');
				$this->set_error('login_unsuccessful');
				$this->set_message('wrong_password');
				return false;
			}
		}
		...
		$this->trigger_events('post_login_unsuccessful');
		$this->set_error('login_unsuccessful');
		$this->set_message('invalid_email');
		return FALSE;
	}
